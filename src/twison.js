var Twison = {
    ...
    /**
     * Extract any referenced images from the provided text.
     *
     * Text containing <img src="url"> would yield an array of images with "url".
     *
     * @param {String} text
     *   The text to examine.
     *
     ...
     */
    extractImagesFromText: function (passage, text, tags) {

        var images = [];

        var matches, expression = new RegExp('((?:https|file)?:\/\/.*?\.(?:png|jpg|webp|avif|gif|jpeg|svg))','igs');
        while ((matches = expression.exec(text)) !== null) {
            images.push(matches[1]);
        }

        var selectors = tags.replace(' ', '|');
        var style = document.getElementById('twine-user-stylesheet').innerText;
        expression = new RegExp('"(?:' + selectors + ')".*?{.*?((?:https|file)?:\/\/.*?\.(?:png|jpg|webp|avif|gif|jpeg|svg)).*?}', 'igs');
        while ((matches = expression.exec(style)) !== null) {
            images.push(matches[1]);
        }
    ...
    convertPassage: function(passage) {
        ...
        const images = Twison.extractImagesFromText(passage, dict.text, passage.attributes["tags"].value);
        if (images) {
            dict.images = images;
        }
